...
class ImgNoteTest(unittest.TestCase):
    ...
    def test_add_imgNote(self):
        ...
        ws_add(self.img_PO, [('i', 1)])
        #是否上传成功
        self.assertTrue(public_check(self.img_PO, self.img_PO.el_img_loc))
        rightClick(self.img_PO, el=self.img_PO.el_imgDIV_loc, action=self.img_PO.btn_imgDel_loc)
        #是否删除成功
    ...
    def test_replace(self):
        ...
        #右键-替换
        rightClick(self.img_PO, el=self.img_PO.el_imgDIV_loc, action=self.img_PO.btn_imgReplace_loc)
    ...
    def test_shear(self):
        ...
        public_init(self.img_PO, self.username, self.password, self.projectName)
        ws_add(self.img_PO, [('i', 1)])
        rightClick(self.img_PO, el=self.img_PO.el_imgDIV_loc, action=self.img_PO.btn_imgCut_loc)
        #检查剪切是否成功
        self.assertFalse(public_check(self.img_PO, self.img_PO.el_imgDIV_loc))
        #剪切后左键点击画布，检查是否会有文件夹多出（BUG点）
        left_click(self.img_PO, 200, 50, el=self.img_PO.tool_loc)
        self.assertEqual(public_check(self.img_PO, self.img_PO.el_divs_loc, islen=True), 2)
        rightClick(self.img_PO, action=self.img_PO.btn_Paste_loc)
        self.assertTrue(public_check(self.img_PO, self.img_PO.el_imgDIV_loc))
    ...
    def test_multiShear(self):
        '''多选剪切，粘贴'''
        public_init(self.img_PO, self.username, self.password, self.projectName)
        ws_add(self.img_PO, [('i', 2)])
        #是否上传成功
        self.assertEqual(public_check(self.img_PO, self.img_PO.el_img_loc, islen=True), 2)
        ...
        self.assertEqual(public_check(self.img_PO, self.img_PO.el_divs_loc, islen=True), 2)
        rightClick(self.img_PO, action=self.img_PO.btn_Paste_loc)
        self.assertEqual(public_check(self.img_PO, self.img_PO.el_imgDIV_loc, islen=True), 2)
    ...
    def test_doubleImgCopy(self):
        ...
        selection(self.img_PO, self.img_PO.el_imgDIV_loc)
        #右键-复制
        rightClick(self.img_PO, el=self.img_PO.el_imgDIV_loc, action=self.img_PO.btn_imgCopy_loc)
        #右键-粘贴
        rightClick(self.img_PO, 450, 10, self.img_PO.el_imgDIV_loc, self.img_PO.btn_Paste_loc)
        #检查个数和图片是否已上传
        self.assertEqual(public_check(self.img_PO, self.img_PO.el_imgDIV_loc, islen=True), 4)
        self.assertTrue(public_check(self.img_PO, self.img_PO.el_img_loc, attr='src'))
        sleep(2)
        ...
        rightClick(self.img_PO, 300, 200, self.img_PO.svg_loc, self.img_PO.btn_Paste_loc)
        self.img_PO.driver.refresh()
        self.assertEqual(public_check(self.img_PO, self.img_PO.el_imgDIV_loc, islen=True), 2)
        self.assertEqual(len(public_check(self.img_PO, self.img_PO.el_img_loc, attr='src')), 2)
